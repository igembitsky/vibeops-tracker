...
// The capture widget runs on the host app's origin and needs exactly two routes
// readable cross-origin: the tag list it renders and the issue it files. Every
// other /api route returns board content, which is reporter emails, diagnostics
// and screenshot bytes. A blanket Access-Control-Allow-Origin let any page the
// operator merely visited read all of it, so the header is granted per route.
function isWidgetRoute(method, pathname) {
  if (method === 'POST' && pathname === '/api/issues') return true;
  return method === 'GET' && /^\/api\/projects\/[^/]+\/tags$/.test(pathname);
}
...
export async function handleApi(req, res, { dataDir }) {
  ...
  // A preflight asks about a different method than the one it carries.
  const intended = req.method === 'OPTIONS'
    ? String(req.headers['access-control-request-method'] || 'GET').toUpperCase()
    : req.method;
  if (isSameOrigin(req) || isWidgetRoute(intended, pathname)) setCors(res);
  ...
  try {
    ...
    } else if ((match = /^POST \/api\/issues\/([^/]+)\/notify$/.exec(route))) {
      // Relay the issue to the host app's notify endpoint (notify_url in projects.json).
      // The tracker stays generic: it never sends mail or knows what "notify the
      // reporter" means; the receiver gets the whole issue plus the operator's note and
      // a kind, and owns the rest (send, then write any receipts back via this API).
      // Same-origin guarded like PATCH/DELETE: this triggers outbound side effects, so
      // a cross-origin drive-by must not be able to fire it. Header-less callers pass.
      if (!isSameOrigin(req)) return sendJson(res, 403, { error: 'cross-origin notify refused' }), true;
      const body = await readJsonBody(req);
      const issue = getIssue(dataDir, decodeURIComponent(match[1]));
      if (!issue) return sendJson(res, 404, { error: `Issue not found: ${match[1]}` }), true;
      const project = getProject(dataDir, issue.project);
      if (!project?.notify_url) return sendJson(res, 400, { error: `No notify_url configured for project: ${issue.project}` }), true;
      try {
        const r = await fetch(project.notify_url, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ issue, kind: body.kind, comment: body.comment, author: body.author }),
          signal: AbortSignal.timeout(20000),
        });
        const out = await r.json().catch(() => null);
        if (r.ok) sendJson(res, 200, out || {});
        else sendJson(res, 502, { error: (out && out.error) || `notify endpoint answered ${r.status}` });
      } catch (e) {
        sendJson(res, 502, { error: `notify endpoint unreachable: ${e.message}` });
      }

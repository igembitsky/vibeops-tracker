...
// Typed, two-sided issue links. Each link is stored on BOTH files with the verb
// that fits its side, so every markdown file is self-describing. 'related' is
// symmetric; 'duplicate_of' (this issue duplicates the canonical one) mirrors to
// 'duplicated_by' (this issue is the canonical survivor) on the other file.
export const LINK_RELS = ['related', 'duplicate_of', 'duplicated_by'];
const REVERSE_REL = { related: 'related', duplicate_of: 'duplicated_by', duplicated_by: 'duplicate_of' };
...
const PATCHABLE = ['status', 'ordinal', 'title', 'tags', 'severity', 'type'];
...
// Attachments live beside the issue markdown, one flat folder per project, named
// after the issue (demo-12.jpg). One image per issue, on purpose: this exists so a
// capture can carry a screenshot of what the reporter was looking at, and a gallery
// would be a different feature. The name is derived from the parsed id, never from
// caller input, so it cannot steer a path.
function attachmentsDir(dataDir, projectKey) {
  return path.join(dataDir, projectKey, 'attachments');
}
...
function serializeIssue(issue) {
  const fm = {
    ...
    last_activity: issue.lastActivity || issue.updated || issue.created,
  };
  if (issue.links?.length) fm.links = issue.links.map((l) => ({ rel: l.rel, to: l.to }));
  if (issue.modifiedFiles?.length) fm.modified_files = issue.modifiedFiles;
  if (issue.screenshot) fm.screenshot = issue.screenshot;
...
// Links live in frontmatter as a list of {rel, to}. Legacy files carry a single
// scalar `related_to` instead; read it as one 'related' link so old files keep
// rendering. This migration is non-destructive: the file only drops related_to
// when it is next written for some other reason (or via linkIssues).
function normalizeRel(rel) {
  return LINK_RELS.includes(rel) ? rel : 'related';
}
function parseLinks(data) {
  if (Array.isArray(data.links)) {
    ...
    for (const l of data.links) {
      if (!l || l.to == null) continue;
      const to = String(l.to);
      if (seen.has(to)) continue; // one relationship per target
      seen.add(to);
      out.push({ rel: normalizeRel(l.rel), to });
    }
    return out;
  }
  if (data.related_to) return [{ rel: 'related', to: String(data.related_to) }];
  return [];
}
...
function parseIssueFile(file) {
  ...
  return {
    ...
    // Last *substantive* activity: created, a status/list move, or a comment.
    // Older files predate the field, so fall back to updated (then created).
    lastActivity: toIso(data.last_activity) || toIso(data.updated) || toIso(data.created) || null,
    links: parseLinks(data),
    modifiedFiles: Array.isArray(data.modified_files) ? data.modified_files.map(String) : [],
    // Filename of the issue's screenshot, when it has one. Files written before
    // attachments existed simply have no field: they read as null, never as broken.
    screenshot: data.screenshot ? String(data.screenshot) : null,
...
export function createIssue(dataDir, input) {
  ...
  const created = withLock(projectLock(dataDir, project), () => {
    ...
    const issue = {
      ...
      screenshot: null, // attached afterwards, by its own route: the image is not JSON
      comments: [],
    };
  ...
  // A related-issue hint (e.g. a regression chain) becomes a proper two-sided
  // 'related' link. Done AFTER the create lock releases, since linkIssues takes
  // the same project lock and it is not reentrant. A dangling target does not
  // fail the create, the link is just skipped.
  if (input.relatedTo) {
    try {
      linkIssues(dataDir, created.id, String(input.relatedTo), 'related');
      return findIssue(dataDir, created.id);
    } catch {
      /* related target missing or invalid: keep the created issue, drop the link */
    }
  }
  return created;
}
...
export function updateIssue(dataDir, id, patch) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    const now = new Date().toISOString();
    issue.updated = now;
    // A status/list move is substantive activity; a quiet edit (title, severity,
    // type, tags, reorder) is not, so only a real status change bumps lastActivity.
    if (issue.status !== prevStatus) issue.lastActivity = now;
    writeFileAtomic(issue.file, serializeIssue(issue));
...
export function resolveIssue(dataDir, id, { resolution, modifiedFiles } = {}) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    issue.lastActivity = issue.updated; // moving to in-review is a status change
    writeFileAtomic(issue.file, serializeIssue(issue));
    return issue;
  });
}
...
// ---- links -----------------------------------------------------------------
// A link is written to BOTH issue files at once, each with the verb for its
// side, so every file is self-describing. Linking is metadata gardening: like
// tag rename it does NOT bump updated/lastActivity, and it does NOT assertOpen
// (relating to, or marking a duplicate of, a closed/iceboxed issue is valid).

// Run fn under the project lock(s) covering both issues. Same project takes one
// lock (withLock is not reentrant, so never nest the same project); cross-project
// takes both in a stable sorted order to avoid deadlock.
function withTwoLocks(dataDir, pA, pB, fn) {
  if (pA === pB) return withLock(projectLock(dataDir, pA), fn);
  const [first, second] = [pA, pB].sort();
  return withLock(projectLock(dataDir, first), () => withLock(projectLock(dataDir, second), fn));
}

function setLink(issue, rel, toId) {
  // One relationship per target: a new link replaces any prior link to toId.
  issue.links = (issue.links || []).filter((l) => l.to !== toId);
  issue.links.push({ rel, to: toId });
  writeFileAtomic(issue.file, serializeIssue(issue));
}

function dropLink(issue, toId) {
  const next = (issue.links || []).filter((l) => l.to !== toId);
  if (next.length === (issue.links || []).length) return; // nothing linked to toId
  issue.links = next;
  writeFileAtomic(issue.file, serializeIssue(issue));
}

export function linkIssues(dataDir, aId, bId, rel = 'related') {
  if (!LINK_RELS.includes(rel)) throw new Error(`Invalid relationship: ${rel} (use ${LINK_RELS.join(', ')})`);
  const pa = parseId(aId);
  const pb = parseId(bId);
  if (!pa) throw notFound(aId);
  if (!pb) throw notFound(bId);
  const reverse = REVERSE_REL[rel];
  return withTwoLocks(dataDir, pa.project, pb.project, () => {
    const a = findIssue(dataDir, aId);
    if (!a) throw notFound(aId);
    const b = findIssue(dataDir, bId);
    if (!b) throw notFound(bId);
    if (a.id === b.id) throw new Error('cannot link an issue to itself');
    setLink(a, rel, b.id);
    setLink(b, reverse, a.id);
    return { a: findIssue(dataDir, a.id), b: findIssue(dataDir, b.id) };
  });
}

export function unlinkIssues(dataDir, aId, bId) {
  const pa = parseId(aId);
  const pb = parseId(bId);
  if (!pa) throw notFound(aId);
  if (!pb) throw notFound(bId);
...
// Enrich a raw [{rel, to}] list with each target's display fields, batched by
// project so a many-link issue reads each project's dir once. A target that no
// longer resolves (deleted) is silently dropped.
export function resolveLinks(dataDir, links) {
  if (!links?.length) return [];
  const indexes = new Map();
  const indexFor = (project) => {
    if (!indexes.has(project)) {
      const idx = new Map();
      for (const i of allProjectIssues(dataDir, project)) idx.set(i.id, i);
      indexes.set(project, idx);
    }
    return indexes.get(project);
  };
  const out = [];
  for (const l of links) {
    const p = parseId(l.to);
    const target = p && indexFor(p.project).get(l.to);
    if (!target) continue;
    out.push({
      rel: l.rel,
      to: l.to,
...
export function deleteIssue(dataDir, id) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    // The issue's attachment goes with it: an orphan image nobody can reach is just
    // disk that grows. Name from the parsed id, same guard as everywhere else.
    for (const k of IMAGE_KINDS) {
      fs.rmSync(path.join(attachmentsDir(dataDir, issue.project), `${issue.id}.${k.ext}`), { force: true });
    }
...
export function iceboxIssue(dataDir, id) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    issue.lastActivity = issue.iceboxed; // parking is a list move
...
export function reviveIssue(dataDir, id) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    issue.updated = new Date().toISOString();
    issue.lastActivity = issue.updated; // reviving to the board is a list move
...
// ---- tags -------------------------------------------------------------------
// Tags are free-form strings in issue frontmatter; there is no tag registry.
// The vocabulary is derived per project from every issue file (open, icebox,
// closed), which keeps the markdown files the single source of truth. Rename
// and delete are maintenance ops: they rewrite tag arrays in place WITHOUT
// bumping `updated`, so gardening the vocabulary never makes issues look
// freshly touched.

function allProjectIssues(dataDir, projectKey) {
  return [
    ...readProjectIssues(dataDir, projectKey),
    ...readProjectIssues(dataDir, projectKey, iceboxDir(dataDir, projectKey)),
    ...readProjectIssues(dataDir, projectKey, closedDir(dataDir, projectKey)),
  ];
}

export function listTags(dataDir, projectKey) {
  const project = sanitizeProjectKey(projectKey);
  const byName = new Map();
  for (const issue of allProjectIssues(dataDir, project)) {
    const touched = issue.updated || issue.created || '';
    for (const name of new Set(issue.tags)) {
      const entry = byName.get(name) || { name, count: 0, lastUsed: '' };
      entry.count += 1;
      if (touched > entry.lastUsed) entry.lastUsed = touched;
...
function validTagName(value, label) {
  const name = String(value ?? '').trim();
  if (!name) throw new Error(`${label} tag name is required`);
  return name;
}

function tagNotFound(name) {
  const err = new Error(`Tag not found: ${name}`);
  err.code = 'NOT_FOUND';
  return err;
}

// Rename `from` to `to` on every issue in the project. When `to` already
// exists this is a merge: issues carrying both end up with one deduped tag.
export function renameTag(dataDir, projectKey, from, to) {
  const project = sanitizeProjectKey(projectKey);
  from = validTagName(from, 'from');
  to = validTagName(to, 'to');
  return withLock(projectLock(dataDir, project), () => {
    const issues = allProjectIssues(dataDir, project);
    const merged = from !== to && issues.some((i) => i.tags.includes(to));
    let retagged = 0;
    for (const issue of issues) {
      if (!issue.tags.includes(from)) continue;
      retagged += 1;
      if (from === to) continue;
      issue.tags = [...new Set(issue.tags.map((t) => (t === from ? to : t)))];
      writeFileAtomic(issue.file, serializeIssue(issue));
    }
    if (!retagged) throw tagNotFound(from);
    return { from, to, retagged, merged };
  });
}

// Remove a tag from every issue in the project.
export function deleteTag(dataDir, projectKey, name) {
  const project = sanitizeProjectKey(projectKey);
  name = validTagName(name, 'tag');
  return withLock(projectLock(dataDir, project), () => {
    let untagged = 0;
    for (const issue of allProjectIssues(dataDir, project)) {
      if (!issue.tags.includes(name)) continue;
      issue.tags = issue.tags.filter((t) => t !== name);
      writeFileAtomic(issue.file, serializeIssue(issue));
      untagged += 1;
    }
    if (!untagged) throw tagNotFound(name);
...
export function addComment(dataDir, id, { author, text }) {
  ...
  return withLock(projectLock(dataDir, parsed.project), () => {
    ...
    issue.updated = new Date().toISOString();
    issue.lastActivity = issue.updated; // a comment is substantive activity
    writeFileAtomic(issue.file, serializeIssue(issue));
    return issue;
  });
}
...
// ---- screenshots -------------------------------------------------------------
// One optional image per issue: what the reporter was looking at when they filed.
// The bytes live in <project>/attachments/<issue-id>.<ext> and the filename is
// recorded in the issue's front matter, so an issue file plus its folder is still
// the whole record (no database, no sidecar index). Written by the capture relay
// (a host app's feedback drain), read by the board.
//
// The extension is decided HERE, by sniffing the magic bytes, never by trusting a
// caller-supplied name or content-type: this is the one place binary from a remote
// app lands on disk.
const IMAGE_KINDS = [
...
export function sniffImage(bytes) {
  ...
  return IMAGE_KINDS.find((k) => k.magic.every((b, i) => bytes[i] === b)) || null;
}
export function imageMime(filename) {
  const ext = String(filename || '').split('.').pop();
  return IMAGE_KINDS.find((k) => k.ext === ext)?.mime || null;
}

// Attach (or replace) an issue's screenshot. Returns the updated issue.
export function saveIssueScreenshot(dataDir, id, bytes) {
  const parsed = parseId(id);
  if (!parsed) throw notFound(id);
  const kind = sniffImage(bytes);
  if (!kind) {
    const err = new Error('screenshot must be a JPEG or PNG');
    err.statusCode = 400;
    throw err;
  }
  return withLock(projectLock(dataDir, parsed.project), () => {
    const issue = findIssue(dataDir, id);
    if (!issue) throw notFound(id);
    const dir = attachmentsDir(dataDir, issue.project);
    fs.mkdirSync(dir, { recursive: true });
    // Name from the PARSED id, so nothing a caller sends steers the path. Replacing
    // an image of the other kind must not leave the old one behind.
    for (const k of IMAGE_KINDS) fs.rmSync(path.join(dir, `${issue.id}.${k.ext}`), { force: true });
    const name = `${issue.id}.${kind.ext}`;
    const file = path.join(dir, name);
...
// The bytes of an issue's screenshot, or null. { bytes, mime } on success.
export function readIssueScreenshot(dataDir, id) {
  const parsed = parseId(id);
  if (!parsed) return null;
  const issue = findIssue(dataDir, id);
  if (!issue?.screenshot) return null;
  const mime = imageMime(issue.screenshot);
  if (!mime) return null;
  // Rebuild the path from the parsed id rather than the stored name: a hand-edited
  // front matter cannot point the reader at an arbitrary file.
  const ext = issue.screenshot.split('.').pop();
  const file = path.join(attachmentsDir(dataDir, issue.project), `${issue.id}.${ext}`);
  try {
    return { bytes: fs.readFileSync(file), mime };
  } catch {
    return null; // recorded but missing: the issue still reads, the image just does not
  }
}
